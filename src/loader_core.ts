...
module loader_core {
    ...
    export class WWALoader {
                    ...
                    try {
                        if (xhr.readyState === XMLHttpRequest.DONE) {
                            if (xhr.status === 200 || xhr.status === 304) {
                                ...
                                throw new Error(
                                    "マップデータ「" + this._fileName + "」が見つかりませんでした。\n" +
                                    "HTTPステータスコードは " + xhr.status + "です。"
                                    );
                            } else if (xhr.status === 403) {
                                throw new Error(
                                    "マップデータ「" + this._fileName + "」を読み取る権限がないようです。\n" +
                                    "管理者の方へ: マップデータのパーミッションを確認してください。\n" +
                                    "HTTPステータスコードは " + xhr.status + "です。"
                                    );
                            } else {
                                throw new Error(
                                    "マップデータ「" + this._fileName + "」の読み込みに失敗しました。\n" +
                                    "HTTPステータスコードは " + xhr.status + "です。"
                                    );
                            }
                        }
        ...
        private _loadAllTextData(): void {
            ...
            for (i = 0; i < this._dataJSObj.message.length; i++) {
                ...
                if (i % 200 === 0) {
                    loader_core.sendProgressToMainProgram(
                        i, this._dataJSObj.message.length,
            ...
            loader_core.sendProgressToMainProgram(
                    this._dataJSObj.message.length, this._dataJSObj.message.length,
                    loader_wwa_data.LoadStage.MESSAGE
                    );
        ...
        public decodeMapData(): DecodeResult {
            ...
            for (
                ...
                if (destCounter + 255 >= destData.length) {
                    var newDestData: Uint8Array =
                        new Uint8Array(destData.length + WWALoader.MEM_BLOCK);
    ...
    export function sendProgressToMainProgram(current: number, total: number, stage: loader_wwa_data.LoadStage): void {
