...
module loader_extractor {
    import PartsType = loader_wwa_data.PartsType;
    export class WWADataExtractor {
        ...
        public extractAllData(): void {
            ...
            this._wwaData.map = this._getFieldDataFromBits(
                loader_wwa_data.PartsType.MAP,
                this._wwaData.mapPartsMax).concat();

            this._wwaData.mapObject = this._getFieldDataFromBits(
                loader_wwa_data.PartsType.OBJECT,
                this._wwaData.objPartsMax).concat();
            ...
            this._wwaData.mapAttribute = this._getPartsDataFromBits(
                loader_wwa_data.PartsType.MAP,
                this._wwaData.mapPartsMax, mapAttrMax).concat();
            this._wwaData.objectAttribute = this._getPartsDataFromBits(
                loader_wwa_data.PartsType.OBJECT,
                this._wwaData.objPartsMax, objAttrMax).concat();
        ...
        private _getFieldDataFromBits( partsType: loader_wwa_data.PartsType, partsMax: number): number[][] {
            ...
            fieldArray = util.new2DArray(
                this._wwaData.mapWidth, this._wwaData.mapWidth);

            for (x = 0; x < this._wwaData.mapWidth; x++) {
                for (y = 0; y < this._wwaData.mapWidth; y++) {
                    if (this._wwaData.version > 29) {
                        fieldArray[x][y] = this._get2BytesNumber(
                            this._currentPosition
                            );
                        this._currentPosition += 2;
                    } else {
                        fieldArray[x][y] = this._get1ByteNumber(
                            this._currentPosition
                            );
                    ...
                    if ( ( x * this._wwaData.mapWidth + y ) % 200 === 0) {
                        loader_core.sendProgressToMainProgram(
                            ...
                            partsType === loader_wwa_data.PartsType.MAP ? loader_wwa_data.LoadStage.MAP_LOAD : loader_wwa_data.LoadStage.OBJ_LOAD
                        );
                    }
                    // 範囲外のパーツ削除
        ...
        private _getPartsDataFromBits(partsType: loader_wwa_data.PartsType, partsMax: number, attrMax: number): number[][] {
            ...
            for (i = 0; i < partsMax; i++) {
                for (j = 0; j < attrMax; j++) {
                    if ( ( i * this._wwaData.mapWidth + j ) % 200 === 0) {
                        loader_core.sendProgressToMainProgram(
                            i * attrMax + j, partsMax * attrMax,
                            partsType === loader_wwa_data.PartsType.MAP ? loader_wwa_data.LoadStage.MAP_ATTR : loader_wwa_data.LoadStage.OBJ_ATTR
                        );
                    }
            ...
            loader_core.sendProgressToMainProgram(
                partsMax * attrMax, partsMax * attrMax,
                partsType === loader_wwa_data.PartsType.MAP ? loader_wwa_data.LoadStage.MAP_ATTR : loader_wwa_data.LoadStage.OBJ_ATTR
                );
            return partsData;
        }
...
*/
        ...
        private _replaceRandomObject(partsID: number, x: number, y: number): void {
            ...
            var afterPartsID: number =
                this._wwaData.objectAttribute[partsID][10 + randomNum];

            if (afterPartsID >= this._wwaData.objPartsMax) { afterPartsID = 0; }
